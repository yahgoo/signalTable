#!/usr/bin/env python3
"""Filter, score, and render Meetup discovery results for SignalTable."""
...
def _load_items(path: str, source_query: str) -> list[dict[str, Any]]:
    ...
    return [normalize_canonical(item, source_query=source_query) for item in items if isinstance(item, dict)]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render scored Meetup discovery report")
    parser.add_argument("--input", "-i", required=True, help="Meetup JSON export")
    parser.add_argument("--query", default="data", help="Source query keyword (data|algorithm|compute)")
